...
def read_words_from_file(inputfile):
    words = [word.strip() for word in open(inputfile, mode='rt')]
    return words


def read_words_from_diceware(inputfile):
    words = [word.split()[1] for word in open(inputfile, mode='rt')]
    return words


def generate(words, ammount, numbers):
    passphrase = []
...
if __name__ == "__main__":
    ...
    if gen_password is True:
        passphrase = generate_password(args.words)
        separator = ''
    else:
        passphrase = generate(words=words, ammount=args.words,
